...
import { PROMPT_ANALYZER_PROMPT_MAX_CHARS, PROMPT_ANALYZER_MAX_TOKENS_MAX, PROMPT_ANALYZER_MAX_TOKENS_MIN, PROMPT_ANALYZER_TEMPERATURE_MAX, PROMPT_ANALYZER_TEMPERATURE_MIN } from '../../shared/prompt-analyzer-types'
import { z } from 'zod'
...
function validateArgs(args: PromptAnalyzerAnalyzeArgs): void {
  const supportedProviders: SupportedProvider[] = ['openrouter', 'openai', 'anthropic', 'google_ai']
  if (!supportedProviders.includes(args.provider)) {
    throw new Error(`Provider ${String(args.provider)} is not supported`)
  }
  if (!args.prompt?.trim()) {
  ...
  if (!Number.isInteger(args.maxTokens) || args.maxTokens < PROMPT_ANALYZER_MAX_TOKENS_MIN || args.maxTokens > PROMPT_ANALYZER_MAX_TOKENS_MAX) {
    throw new Error('Prompt analyzer max tokens must be between 1 and 32768')
  }
  if (!Number.isFinite(args.temperature) || args.temperature < PROMPT_ANALYZER_TEMPERATURE_MIN || args.temperature > PROMPT_ANALYZER_TEMPERATURE_MAX) {
